...
check(Matrix, Direction, XPos, YPos, Passed) -> case sets:is_element({Direction, XPos, YPos}, Passed) of
                                                    true -> stuck;
                                                    false -> case lookahead(Matrix, Direction, XPos, YPos) of
                                                        out -> out;
                                                        obstructed -> check(Matrix, turn(Direction), XPos, YPos, Passed);
                                                        free -> move(Matrix, Direction, XPos,  YPos, sets:add_element({Direction, XPos, YPos}, Passed))
                                                     end
                                            end.
...
main() ->
    ...
    CrossProduct = [{XX, YY} || {XX, _} <- lists:enumerate(lists:nth(1, Matrix)), {YY, _} <- lists:enumerate(Matrix)],
    Candidates = lists:filter(fun ({XX, YY}) -> ?NTH(Matrix, XX, YY) == $. end, CrossProduct),
    Escapes = lists:map(fun ({XX, YY}) -> check(obstruct(Matrix, XX, YY), up, X, Y, sets:new()) end, Candidates),
