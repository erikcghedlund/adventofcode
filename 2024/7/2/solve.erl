...
number_concat(Int1, Int2) ->
    help:fst(string:to_integer(integer_to_list(Int1) ++ integer_to_list(Int2))).

eval([Head | Tail]) -> eval(Tail, [Head]).
eval([Int], Accum) ->
    lists:map(fun(X) -> X + Int end, Accum) ++ lists:map(fun(X) -> X * Int end, Accum) ++
        lists:map(fun(X) -> number_concat(X, Int) end, Accum);
eval([Head | Tail], Accum) ->
    eval(
        Tail,
        lists:map(fun(X) -> X + Head end, Accum) ++ lists:map(fun(X) -> X * Head end, Accum) ++
            lists:map(fun(X) -> number_concat(X, Head) end, Accum)
    ).
