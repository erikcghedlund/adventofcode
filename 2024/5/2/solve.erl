...
main() ->
    ...
    InvalidManuals = lists:filter(
        fun(Manual) -> not manual_is_valid(AfterMap, Manual) end, ConvertedManuals
    ),
    SortedManuals = lists:map(
        fun(Manual) -> lists:sort(fun(A, B) -> sort_fun(AfterMap, A, B) end, Manual) end,
        InvalidManuals
    ),
    io:format("~p\n", [
