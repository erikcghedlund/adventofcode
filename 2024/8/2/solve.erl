...
populate_map(Matrix, _, Y, Map) when length(Matrix) < Y ->
    maps:filter(fun(_, Val) -> length(Val) > 1 end, Map);
...
get_anti(TwoCombs, Width, Height) ->
    Foo = fun({Point1, Point2}) ->
        ...
        [
            get_anti_helper(Point1, Dist, Width, Height, minus),
            get_anti_helper(Point2, Dist, Width, Height, plus)
        ]
    end,
