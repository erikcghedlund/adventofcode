...
dfs(Matrix, {CurX, CurY}, To) ->
    ...
    case lists:filter(fun({_, N}) -> N == Val + 1 end, Neigbours) of
        [] -> false;
        Paths -> lists:map(fun({P, _}) -> dfs(Matrix, P, To) end, Paths)
    end.
...
main() ->
    ...
    Paths = lists:flatten((plists:pmap(fun({From, To}) -> dfs(ArrayMatrix, From, To) end, AllTests))),
    io:format("~p\n", [help:count(true, Paths)]),
    halt().
