...
move(List, Accum) ->
    case not lists:suffix(lists:filter(fun isempty/1, List), List) of
        true ->
            ...
            ToRecurse =
                lists:reverse(
                    lists:subtract(lists:reverse(lists:nthtail(DropLen, ToWork)), ToAppend)
                ) ++ lists:duplicate(DropLen, empty),
