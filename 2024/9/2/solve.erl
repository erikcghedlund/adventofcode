...
move(List, Index) ->
    ...
    ICandidates = [ {I, Occurences} || {I, {Val, Occurences}} <- lists:enumerate(Candidates), Val == empty],
    NewList = 
    case lists:search(fun ({_, Occurences}) -> RequiredSpace =< Occurences end, ICandidates) of
        ...
        {value, {I, Occurences}} ->
            ...
            case Occurences - RequiredSpace of
                0 -> KeepHead ++ [{Index, RequiredSpace}] ++ BeforeIndex ++ [{empty, RequiredSpace}] ++ AfterIndex;
                N -> KeepHead ++ [{Index, RequiredSpace}] ++ [{empty, N}] ++ BeforeIndex ++ [{empty, RequiredSpace}] ++ AfterIndex
            end
    end,
